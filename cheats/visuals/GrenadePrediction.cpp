...
void GrenadePrediction::Paint()
{
	...
	if (he_target.second && weapon->m_iItemDefinitionIndex() == WEAPON_HEGRENADE) {
		Vector screen;
		...
		if (math::world_to_screen(prev, screen))
			render::get().text(fonts[ESP], screen.x - 4, screen.y - 28, (int)he_target.first >= he_target.second->m_iHealth() ? Color::Red : Color::White, HFONT_CENTERED_X | HFONT_CENTERED_Y, text.c_str());
	}
	else if (molo_target.second && weapon->m_iItemDefinitionIndex() == (WEAPON_MOLOTOV || WEAPON_INCGRENADE)) {
		Vector screen;
		...
		std::string text = (std::to_string(molo_target.first) + " m");

		if (math::world_to_screen(prev, screen))
			render::get().text(fonts[ESP], screen.x - 4, screen.y - 28, (int)molo_target.first >= molo_target.second->m_iHealth() ? Color::Red : Color::White, HFONT_CENTERED_X | HFONT_CENTERED_Y, text.c_str());
	}
	...
	if (type && path.size() > 1)
	{
		...
		for (auto it = path.begin(); it != path.end(); ++it)
		{
			if (math::world_to_screen(prev, nadeStart) && math::world_to_screen(*it, nadeEnd))
				render::get().line((int)nadeStart.x, (int)nadeStart.y, (int)nadeEnd.x, (int)nadeEnd.y, trace_color);
