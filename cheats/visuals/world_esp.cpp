...
void worldesp::molotov_timer(entity_t* entity)
{
	...
	if (isOnScreen(origin, screen_origin))
	{
		Vector vOutStart;
		if (math::world_to_screen(origin, vOutStart)) {
			render::get().filled_circle(vOutStart.x, vOutStart.y, 23, 26, Color(15, 15, 15, 187));
			render::get().text(fonts[GRENADES], vOutStart.x, vOutStart.y - 16, Color(255, 255, 255, 255), HFONT_CENTERED_X, "l");
			std::string dmg = ""; dmg += std::to_string(int(max(g_ctx.local()->GetAbsOrigin().DistTo(origin) / 3.28, 0))); dmg += "m";
			render::get().text(fonts[NAME], vOutStart.x, vOutStart.y + 1, Color(255, 255, 255), HFONT_CENTERED_X, dmg.c_str());
			render::get().CircularProgressBar(vOutStart.x, vOutStart.y, 22, 23, 90, 360 * factor, Color(255, 255, 255, 255), false);
		}
	}
	...
	if (!isOnScreen(origin, screen_origin))
	{
		...
		auto angleYawRad = DEG2RAD(viewAngles.y - math::calculate_angle(g_ctx.globals.eye_pos, origin).y - 90.0f);
		...
		math::rotate_triangle(points, viewAngles.y - math::calculate_angle(g_ctx.globals.eye_pos, origin).y - 90.0f);

		render::get().filled_circle(warn.x, warn.y, 23, 26, Color(15, 15, 15, 187));
		render::get().text(fonts[GRENADES], warn.x, warn.y - 16, Color(255, 255, 255, 255), HFONT_CENTERED_X, "l");
		std::string dmg = ""; dmg += std::to_string(int(max((g_ctx.local()->GetAbsOrigin().DistTo(origin)) / 3.28, 0))); dmg += "m";
		render::get().text(fonts[NAME], warn.x, warn.y + 1, Color(255, 255, 255), HFONT_CENTERED_X, dmg.c_str());
		render::get().CircularProgressBar(warn.x, warn.y, 22, 23, 90, 360 * factor, Color(255, 255, 255, 255), false);
	}
	...
	auto test = inferno->m_hOwnerEntity();

	if (test->m_iTeamNum() == g_ctx.local()->m_iTeamNum() && m_cvar()->FindVar("mp_friendlyfire")->GetBool() == 0)
		render::get().text(fonts[ESP], screen_origin.x, screen_origin.y + 30, Color(100, 200, 0, 255), HFONT_CENTERED_X, "safe");
	else
		render::get().text(fonts[ESP], screen_origin.x, screen_origin.y + 30, Color(255, 0, 0, 255), HFONT_CENTERED_X, "unsafe");

	render::get().text(fonts[ESP], screen_origin.x, screen_origin.y + 38, Color(255, 255, 255, 255), HFONT_CENTERED_X, "molly");

	Vector mins, maxs;
	inferno->GetClientRenderable()->GetRenderBounds(mins, maxs);

	render::get().Draw3DCircle(origin, Vector(maxs - mins).Length2D() * 0.5, Color(g_cfg.esp.molotov_timer_color.r(), g_cfg.esp.molotov_timer_color.g(), g_cfg.esp.molotov_timer_color.b()));
}
...
void worldesp::smoke_timer(entity_t* entity)
{
	...
	if (isOnScreen(origin, screen_origin))
	{
		...
		if (math::world_to_screen(origin, vOutStart)) {
			render::get().filled_circle(vOutStart.x, vOutStart.y, 23, 26, Color(15, 15, 15, 187));
			render::get().text(fonts[GRENADES], vOutStart.x, vOutStart.y - 16, Color(255, 255, 255, 255), HFONT_CENTERED_X, "k");
			std::string dmg = ""; dmg += std::to_string(int(max(g_ctx.local()->GetAbsOrigin().DistTo(origin) / 3.28, 0))); dmg += "m";
			render::get().text(fonts[NAME], vOutStart.x, vOutStart.y + 1, Color(255, 255, 255), HFONT_CENTERED_X, dmg.c_str());
			render::get().CircularProgressBar(vOutStart.x, vOutStart.y, 22, 23, 90, 360 * factor, Color(255, 255, 255, 255), false);
		}
	...
	if (!isOnScreen(origin, screen_origin))
	{
		...
		render::get().filled_circle(warn.x, warn.y, 23, 26, Color(15, 15, 15, 187));
		render::get().text(fonts[GRENADES], warn.x + 2, warn.y - 16, Color(255, 255, 255, 255), HFONT_CENTERED_X, "k");
		std::string dmg = ""; dmg += std::to_string(int(max((g_ctx.local()->GetAbsOrigin().DistTo(origin)) / 3.28, 0))); dmg += "m";
		render::get().text(fonts[NAME], warn.x, warn.y + 1, Color(255, 255, 255), HFONT_CENTERED_X, dmg.c_str());
		render::get().CircularProgressBar(warn.x, warn.y, 22, 23, 90, 360 * factor, Color(255, 255, 255, 255), false);
	}
	...
	render::get().Draw3DCircle(origin, 144.f, Color(g_cfg.esp.smoke_timer_color.r(), g_cfg.esp.smoke_timer_color.g(), g_cfg.esp.smoke_timer_color.b()));
}
