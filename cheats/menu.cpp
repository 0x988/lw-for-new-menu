	...
	else
	{
		...
		else
		{
			...
			ImGui::BeginChild(crypt_str("##RAGE2_SECOND"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				...
				ImGui::Checkbox(crypt_str("Air shot"), &g_cfg.ragebot.weapon[hooks::rage_weapon].air_shot);
				if (g_cfg.ragebot.weapon[hooks::rage_weapon].air_shot)
					ImGui::SliderInt(crypt_str("Air hitchance amount"), &g_cfg.ragebot.weapon[hooks::rage_weapon].air_hitchance_amount, 1, 100);
	...
	if (!child)
	{
		...
		else
		{
			...
			ImGui::BeginChild(crypt_str("##MISC1_SECOND"), ImVec2(317 * dpi_scale, (child_height - 35) * dpi_scale));
			{
				...
				ImGui::Checkbox(crypt_str("ModelScale"), &g_cfg.menu.modelscale);
				...
				if (g_cfg.menu.keybinds) {
					ImGui::SliderInt(crypt_str("Keybind X"), &g_cfg.menu.keybind_x, 0, 2048);
					ImGui::SliderInt(crypt_str("Keybind Y"), &g_cfg.menu.keybind_y, 0, 2048);
				}
...
void c_menu::draw(bool is_open)
{
	...
	if (logo == nullptr)
		D3DXCreateTextureFromFileInMemoryEx(device, logotype, sizeof(logotype), 90, 90, D3DX_DEFAULT, D3DUSAGE_DYNAMIC, D3DFMT_UNKNOWN, D3DPOOL_DEFAULT, D3DX_DEFAULT, D3DX_DEFAULT, 0, NULL, NULL, &logo);

	if (foto == nullptr)
		D3DXCreateTextureFromFileInMemoryEx(device, fototype, sizeof(fototype), 45, 45, D3DX_DEFAULT, D3DUSAGE_DYNAMIC, D3DFMT_UNKNOWN, D3DPOOL_DEFAULT, D3DX_DEFAULT, D3DX_DEFAULT, 0, NULL, NULL, &foto);
	...
	IconFont = io.Fonts->AddFontFromMemoryTTF(Icons, sizeof(Icons), 22, NULL, /*io.Fonts->GetGlyphRangesCyrillic()*/ NULL);
	/*LexendDecaFont = io.Fonts->AddFontFromMemoryTTF(LexendDeca, sizeof(LexendDeca), 22, NULL, io.Fonts->GetGlyphRangesCyrillic());
	InterMedium = io.Fonts->AddFontFromMemoryTTF(Intermedium, sizeof(Intermedium), 17, NULL, io.Fonts->GetGlyphRangesCyrillic()); */ // me: crash? = idk)
	...
	if (ImGui::Begin(("###"), nullptr, ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoScrollbar))
	{
		...
		ImGui::PushFont(InterMedium);
		ImGui::GetWindowDrawList()->AddText(ImVec2(p.x + 580, p.y + 20), ImColor(83, 86, 93), "Version:");// search, x - up, y - down.
		ImGui::GetWindowDrawList()->AddText(ImVec2(p.x + 640, p.y + 20), ImColor(155, 212, 90), "beta");// search, x - up, y - down.
		ImGui::PopFont();
	}
	ImGui::End();
