...
void eventlogs::add(std::string text, bool full_display)
{
	...
	if (g_cfg.misc.log_output[EVENTLOG_OUTPUT_CHAT])
	{
		...
		if (!chat)
			...
			// \x01 - дефолтный цвет белый вроде
			// \x02 - красный
			// \x03 - фиолетовый
			// \x04 - €рко зеленый
			// \x05 - тускло зеленый
			// \x06 - пиздец зеленый заболел и побледнел :|
			// \x07 - тусклый красный
			// \x08 - серый
			// \x09 - желтый
			// \x0A - габен ебанулс€ серый чуть чернее сделал
			// \x0C - синий
			// \x10 - оранжевый(это пиздец он на желтый похож)
