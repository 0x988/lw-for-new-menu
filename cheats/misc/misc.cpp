...
void misc::watermark()
{
	...
	auto watermark = crypt_str("Legendware | ") + computer_name() + crypt_str(" | ") + g_ctx.globals.time;

	if (m_engine()->IsInGame())
	{
		char const* server_type = "offline";

		if (m_engine()->IsPlayingDemo())
			server_type = "demo";
		else if (m_engine()->IsClientLocalToActiveServer())
			server_type = "local";
		else if (m_engine()->IsInGame())
		{
			auto info = m_engine()->GetNetChannelInfo();

			if (m_engine()->IsHLTV())
			{
				if (m_gamerules() && m_gamerules()->m_bIsValveDS())
		...
		watermark = crypt_str("Legendware | ") + computer_name() + crypt_str(" | ") + server_type + crypt_str(" | ") + std::to_string(g_ctx.globals.ping) + crypt_str(" ms | ") + tickrate + crypt_str(" tick | ") + g_ctx.globals.time;
	}
	auto box_width = render::get().text_width(fonts[NAME], watermark.c_str()) + 10;

	render::get().gradient(width - 10 - box_width, 9, box_width / 2, 2, Color(0, 0, 0, 0), Color(g_cfg.menu.menu_theme.r(), g_cfg.menu.menu_theme.g(), g_cfg.menu.menu_theme.b(), g_cfg.menu.menu_theme.a()), GRADIENT_HORIZONTAL);
	render::get().gradient(width - 10 - (box_width / 2), 9, box_width, 2, Color(g_cfg.menu.menu_theme.r(), g_cfg.menu.menu_theme.g(), g_cfg.menu.menu_theme.b(), g_cfg.menu.menu_theme.a()), Color(0, 0, 0, 0), GRADIENT_HORIZONTAL);

	render::get().rect_filled(width - 10 - box_width, 11, box_width, 18, Color(10, 10, 10, 215));
