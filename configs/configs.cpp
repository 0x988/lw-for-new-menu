...
void C_ConfigManager::setup()
{
	...
	for (auto i = 0; i < 8; i++)
	{
		...
		setup_item(&g_cfg.ragebot.weapon[i].air_shot, false, std::to_string(i) + crypt_str("Ragebot.air_shot"));
		setup_item(&g_cfg.ragebot.weapon[i].air_hitchance_amount, 1, std::to_string(i) + crypt_str("Ragebot.air_hitchance_amount"));
	}
	...
	setup_item(&g_cfg.misc.forcecrosshair, false, crypt_str("Misc.forcecrosshair"));
	...
	setup_item(&g_cfg.menu.modelscale, false, crypt_str("menu.modelscale"));
	setup_item(&g_cfg.menu.keybinds, true, crypt_str("Menu.keybinds"));
	setup_item(&g_cfg.menu.keybind_x, 0, crypt_str("Menu.keybinds_x"));
	setup_item(&g_cfg.menu.keybind_y, 0, crypt_str("Menu.keybinds_y"));
